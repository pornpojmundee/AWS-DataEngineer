...
| Kinesis + Lambda streaming | 🔲 Code written, not deployed | `pipelines/streaming/` has a working producer/consumer pair; deployment blocked — the AWS account hits the same "Complete your account setup" gate on Kinesis as it does on Redshift, despite passing general payment/identity verification (S3, Glue, Athena, IAM all work normally). This appears to be a separate, service-specific approval AWS applies to a subset of services |
| Redshift Serverless | 🔲 Designed, not deployed | Same account-level block as Kinesis above; schema/query plan described earlier in this README |
